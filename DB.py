...
class DB(object):
    ...
    def get_user_ids(self):
        try:
            q_user_ids = "SELECT DISTINCT user_id FROM USER_ACTIONS;"
            df_user_ids = pd.read_sql_query(q_user_ids,self.conn)
            logging.debug("Obtained user ids from USER_ACTIONS table")
            return df_user_ids
        except Exception as e:
            logging.error('Could not obtain user id from Users table: ' + str(e))
    ...
    def get_count_claim_types(self,user_id,claim_type):
        try:
            q_count = "SELECT COUNT(*) FROM USER_ACTIONS WHERE object = "+"'dropdown claim types'"+" AND value = "+"'{}'".format(claim_type)+" AND user_id = "+str(user_id)
            df_count = pd.read_sql_query(q_count,self.conn)
            print("inside fn ",q_count,df_count)
            logging.debug("Obtained count of claim type {} from USERS_ACTION table".format(claim_type))
            return df_count
        except Exception as e:
            logging.error("Could not obtain count of claim type {} from USERS_ACTION table: ".format(claim_type) + str(e))  
