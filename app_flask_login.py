...
app2 = dash.Dash(__name__, server=server, url_base_pathname='/analytics/')
...
app2.layout = html.Div([dcc.Link('Go to main page', href='/success'),
                        html.H1('Analytics'),
                       dcc.Dropdown(id='dropdown_user_id',
                                    options=[{'label':i, 'value':i} for i in df_user_ids['user_id']],
...
# Tab 1 callback
@app1.callback([Output('table_claims', 'data'),
...
def change_dropdown_claim_type(n_clicks,claim_types,claim_status,claim_number,start_date,end_date):
    ...
    if trigger_id == 'go-val': #if 'go' button was clicked
        ...
        if lv_start_date and lv_end_date:
            print(lv_start_date,lv_end_date)
...
@app1.callback(Output('table_claim_details', 'data'),
...
# Analytics callbacks
# Dropdown user id
@app2.callback(Output('display_selected_user_id','children'),
              Input('dropdown_user_id','value'))
def display_selected_user_id(user_id):
    if user_id:
        fire = db.get_count_claim_types(user_id,'fire')["COUNT(*)"][0]
        water = db.get_count_claim_types(user_id,'water')["COUNT(*)"][0]
        theft = db.get_count_claim_types(user_id,'theft')["COUNT(*)"][0]
        return dcc.Graph(
                            id='graph_claim_types',
                            figure={
                                'data': [
                                    {'x': ['fire','water','theft'], 'y': [fire, water, theft], 'type': 'bar', 'name': 'SF'},
                                ],
                                'layout': {
